...
セッションスコープストアは、Runtimeが確定したセッションID配下のworkspaceディレクトリに置く。`SESSION_ID` と `RESEARCH_SESSION_STORE_DIR` をKB Retriever / S3 Fetcher / Citation Verifierの各MCPサーバenvへ注入し、別プロセス間で同じ保存先を参照できるようにする。
...
- Citation VerifyはLLMが渡したcontextsではなく、Retrieverが保存したセッションスコープのcontextを検証する
